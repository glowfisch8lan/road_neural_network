...
# убираем необходимое количество знаков
...
def direction_wind_to_matrix(float):  # Направление ветра
    matrix = {'С': 0, 'СВ': 0, 'В': 0, 'ЮВ': 0, 'Ю': 0, 'ЮЗ': 0, 'З': 0, 'СЗ': 0}
    if 22.5 >= float >= 0:
        matrix['С'] = 1
...
def to_binary_range(value, ranges, to_float):
    ...
    for range in ranges:
        if range[0] <= value <= range[1]:
            ...
            str += '0'
    if to_float:
        arr = array_str_to_float(str)
        return arr
    arr = array_str_to_number(str)
    return arr
